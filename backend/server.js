...
mongoose.connect("mongodb+srv://root:123@cluster0.oej9r.mongodb.net/crud?retryWrites=true&w=majority",{ useNewUrlParser: true, useCreateIndex: true, useUnifiedTopology: true });
var fs = require('fs');
  ...
  try {
    if (req.body && req.body.username && req.body.password) {
      user.find({ username: req.body.username }, (err, data) => { 
        if (data.length > 0) {         
          if (bcrypt.compareSync(data[0].password, pwd)) {
